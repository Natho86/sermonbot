...
- **Google Secret Manager**: You must create the following secrets and store their values (not the names) in Secret Manager **before deploying**:
  - `llec-sermonbot-gcp-sa-account`: JSON key file for the service account
  - `llec-sermonbot-wp-app-password`: WordPress application password
  ...
  - `llec-sermonbot-api-key`: A secure random string to use as the API key
    ```bash
    # Generate a secure random API key (32 bytes, base64 encoded)
    API_KEY=$(openssl rand -base64 32)

    # Create the secret
    gcloud secrets create llec-sermonbot-api-key --replication-policy="automatic"

    # Add the API key to the secret
    echo -n "$API_KEY" | gcloud secrets versions add llec-sermonbot-api-key --data-file=-
...
```yaml
TEMP_BUCKET_NAME: "llec-sermonbot-temp"
SERVICE_ACCOUNT_SECRET: "llec-sermonbot-gcp-sa-account"
WORDPRESS_APP_PASSWORD_SECRET: "llec-sermonbot-wp-app-password"
...
API_KEY_SECRET: "llec-sermonbot-api-key"
...
BUCKET_NAME="llec-sermonbot-temp"
...
API_KEY=$(gcloud secrets versions access latest --secret=llec-sermonbot-api-key)
